...
def count_number_successful():
    ...
    while True:
        ...
        if (total == 1 or total % 1000 == 0) and C.is_solved():
            print("%s: %s successes (%.3f%% passing)" % (int(total), successes, 100 * successes / total), end=' ')
            print("moves=%s avg_moves=%0.3f" % (len(solver.moves), avg_moves), end=' ')
            print("opt_moves=%s avg_opt_moves=%0.3f" % (len(opt_moves), avg_opt_moves), end=' ')
